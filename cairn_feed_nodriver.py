...
import nodriver  # Importation de Nodriver pour le contrôle du navigateur
import logging
import pickle
import asyncio
...
class CairnFeedGenerator:
    ...
    async def _get_html_content(self):
        """Utilise Nodriver pour récupérer le contenu HTML de la page des publications."""
        # Lancement du navigateur Nodriver
        browser = await nodriver.launch(headless=True, args=['--no-sandbox'])
    ...
    async def generate_feed(self):
        """Génère le flux Atom complet en utilisant Nodriver pour récupérer le contenu."""
        try:
