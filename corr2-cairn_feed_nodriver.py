...
class CairnFeedGenerator:
    ...
    def parse_date(self, date_text):
        ...
        try:
            ...
            logging.warning(f"Impossible de parser la date: {date_text}. {e}")
    ...
    async def extract_publication_info(self, publication):
        ...
        try:
            if not publication:
                logging.warning("Publication invalide (None)")
                return None
            ...
            if title_elem is None:
                logging.warning("Titre manquant dans la publication")
                return None
            ...
            if link_elem is None:
                logging.warning("Lien manquant dans la publication")
                return None
            url = await link_elem.evaluate('element => element.href')
            if not url:
                logging.warning("URL invalide dans la publication")
                return None
            ...
            if not all([title_text, full_url]):
                logging.warning("Données essentielles manquantes dans la publication")
                return None
    ...
    async def generate_feed(self):
        ...
        try:
            async with self._browser_context() as browser:
                ...
                if not publications:
                    logging.warning("Aucune publication trouvée")
                    return
